...
Slurpy is a cross-platform desktop speech-to-text application built with Tauri 2.x (Rust backend + React/TypeScript frontend).
...
Slurpy supports command-line parameters on all platforms for integration with scripts, window managers, and autostart configurations.
...
Slurpy is a personal fork of [cjpais/Handy](https://github.com/cjpais/Handy)
(MIT). Upstream's contribution rules — the feature freeze, the
gather-community-support-first policy, their Discussions — govern *upstream*,
not this repo. The templates below are inherited from upstream and kept
because their discipline is good, but the feature freeze does not apply here.

Names that deliberately stay "handy" and must not be rebranded: the crate and
binary (`handy`, so CLI flags and tooling keep working), the `handy-keys`
dependency and its `HandyKeys` keyboard implementation (a persisted settings
value), the `handy-computer` Hugging Face org in model ids, the
`"Handy Portable Mode"` marker string, and the legacy `com.pais.handy`
app-data path used by the one-time migration.
...
- **Opening an issue:** Read [`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/) and pick the right template (`bug_report.md` for bugs).
- **Proposing a feature:** No feature freeze applies to this fork. Upstream's freeze and its [Discussions](https://github.com/cjpais/Handy/discussions) process apply only to cjpais/Handy.
