...
///
/// This intentionally describes the transcription output, not Slurpy's UI
...
/// Filler tokens that are not lexical words in any language Slurpy's models can
...
mod tests {
    ...
    fn test_apply_custom_words_handles_unicode_punctuation() {
        let text = "「Slurpee。」";
        let custom_words = vec!["Slurpy".to_string()];
        let result = apply_custom_words(text, &custom_words, 0.5);
        assert_eq!(result, "「Slurpy。」");
    }
